...
export default function ConfirmEmailChange() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // OpenAPI: POST /api/v1/auth/confirm-email-change/ {token}
      const res = await api.confirmEmailChange(token.trim());
      setMessage(res?.detail || (language === "sw" ? "Email imebadilishwa kikamilifu." : "Email changed successfully."));
      setTimeout(() => navigate("/login?email_changed=1", { replace: true }), 900);
    } catch (err) {
      setError(err?.message || (language === "sw" ? "Imeshindikana kuthibitisha mabadiliko ya email." : "Failed to confirm email change."));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <SEOHead title={language === "sw" ? "Thibitisha Kubadili Email" : "Confirm Email Change"} description="" />
      ...
      <section className={`min-h-[calc(100vh-160px)] flex items-center justify-center px-4 py-10 ${isDark ? "bg-gray-950" : "bg-gray-50"}`}>
        <div className={`max-w-md w-full rounded-2xl shadow-xl border p-6 md:p-8 ${isDark ? "bg-gray-900 border-gray-800" : "bg-white border-gray-100"}`}>
          <h1 className={`text-lg md:text-xl font-bold ${isDark ? "text-white" : "text-gray-900"}`}>
            {language === "sw" ? "Thibitisha Kubadili Email" : "Confirm Email Change"}
          </h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-4">
            <div>
              ...
              <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-2">
                {language === "sw"
                  ? "Kwa kawaida token huja kwenye link ya email (mfano: ?token=...)."
                  : "Token usually comes from the email link (e.g. ?token=...)."}
              </p>
            </div>
            ...
            <div className="flex items-center justify-between pt-1">
              ...
              >
                {loading ? (language === "sw" ? "Inathibitisha..." : "Confirming...") : (language === "sw" ? "Thibitisha" : "Confirm")}
              </button>
            </div>
          </form>
        </div>
